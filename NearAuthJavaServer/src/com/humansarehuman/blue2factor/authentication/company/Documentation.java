...
@Controller
@RequestMapping(Urls.DOCUMENTATION)
@SuppressWarnings("ucd")
public class Documentation extends BaseController {
	...
	public String processGet(HttpServletRequest request, HttpServletResponse httpResponse, ModelMap model) {
		...
		if (incorrect != null && incorrect.equals("1")) {
			...
			if (adminSignin.isAllowed()) {
				...
				companyResponseHelper = this.showDocPage(adminSignin.getGroup(), request, httpResponse, model);
			} else {
				dataAccess.addLog("Documentation", "not allowed");
	...
	public String documentationProcessPost(HttpServletRequest request, HttpServletResponse httpResponse, ModelMap model) {
		CompanyResponseHelper companyResponseHelper;
		AdminSignin adminSignin = checkPermission(request, httpResponse);
		if (adminSignin.isAllowed()) {
			updateCompanyPageCookie(httpResponse, adminSignin.getGroup());
		...
		model = companyResponseHelper.getModel();
		model.addAttribute("pageTitle", "NearAuth.ai Documentation");
		model.addAttribute("action", "/documentation");
	...
	public CompanyResponseHelper showDocPage(GroupDbObj group, HttpServletRequest request,
